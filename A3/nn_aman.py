...
class layer:
    def __init__(self, n_units, layer_num=0, up_layer=None, down_layer=None):
        ...
        self.net = (
            None  # net for layer -> (M x n_units) matrix -> basically input * theta.T
        )
...
class neural_network:
    ...
    def init_theta(self, layer):
        if layer.layer_num == 0:
            ...
            weight = np.random.normal(
                0,
                math.sqrt(1 / layer.up_layer.n_units),
                (layer.n_units, layer.up_layer.n_units),
            )
            bias = np.zeros((layer.n_units, 1))
            layer.theta = np.concatenate((bias, weight), axis=1)
    ...
    def init_network(self):
        ...
        for i in range(len(self.hidden_layer) - 1):
            hidden_layer = layer(
                self.hidden_layer[i + 1], layer_num=i + 1, up_layer=self.layers[-1]
            )
        ...
        output_layer = layer(
            self.r,
            layer_num=len(self.hidden_layer) + 1,
            up_layer=self.layers[-1],
            down_layer=None,
        )
    ...
    def softmax(self, x, theta):
        ...
        )  # size - M x n_units (specifically Mx5)
        return output, net
    ...
    def forward_prop(self, x):
        ...
        for layer in self.layers:
            if (
                layer != self.end
            ):  # checking if the layer has a non None down layer (i.e. basically avoiding output layer)
                layer.output, layer.net = self.activation(
                    layer.input, layer.theta
                )  # output
                layer.down_layer.input = self.add_bias(
                    layer.output
    ...
    def backward_prop(self, y):  # NOTE - y is one hot encoded
        ...
        self.end.grad_theta = np.dot(
            self.end.delta.T, self.end.input
        )  # size - r x (prev_n_units+1)
        ...
        for layer in reversed(self.layers[:-1]):
            ...
            layer.delta = (
                layer.down_layer.delta @ layer.down_layer.theta[:, 1:]
            ) * self.activation_derivative(layer)
            # print(layer.delta.shape)
            layer.grad_theta = np.dot(
                layer.delta.T, layer.input
            )  # size - n_units x (prev_n_units+1)
    ...
    def predict(self, x, y_test=None):
        ...
        prediction = (
            np.argmax(output, axis=1) + 1
        )  # +1 because argmax index starts from 0, but predictions start from 1
        return prediction
...
if __name__ == "__main__":
    ...
    x_train_path = os.path.join(dirname, "data/q2/x_train.npy")
    y_train_path = os.path.join(dirname, "data/q2/y_train.npy")
    ...
    x_test_path = os.path.join(dirname, "data/q2/x_test.npy")
    y_test_path = os.path.join(dirname, "data/q2/y_test.npy")
